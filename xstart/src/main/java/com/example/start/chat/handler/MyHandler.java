...
import com.example.start.chat.processor.InformationOperateMap;
    ...
    private void handleWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) throws Exception {
        ...
        if ((frame instanceof TextWebSocketFrame)) {
            ...
            if (InformationOperateMap.isNo(mage)) {
                //判断是否有这个聊天室
                if (InformationOperateMap.map.containsKey(mage.getChatRoom())) {
                    //判断是否有其他用户
                    if (InformationOperateMap.map.get(mage.getChatRoom()).size() > 0) {
                        InformationOperateMap.map.get(mage.getChatRoom()).forEach((id, iom) -> {
                            try {
                                IMMessage mag = iom.getMage();
                                mag.setContent("30003");
                                //发送其他用户信息给要注册用户
