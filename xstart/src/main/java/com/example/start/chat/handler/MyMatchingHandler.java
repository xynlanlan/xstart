...
import com.example.start.chat.processor.InformationOperateMapMatching;
...
import static com.example.start.chat.processor.InformationOperateMapMatching.queue;
...
public class MyMatchingHandler extends SimpleChannelInboundHandler<Object> {
    ...
    public void close(ChannelHandlerContext ctx, ChannelPromise promise) throws Exception {
        ...
        //关闭连接将移除该用户消息
        IMMessage mage = new IMMessage();
        mage.setSender(this.name);
        mage.setContent("20002");
        //将用户下线信息发送给为下线用户
    ...
    private void handleWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) throws Exception {
        ...
        if ((frame instanceof TextWebSocketFrame)) {
            ...
            //消息转成Mage
            IMMessage mage = IMMessage.strJson2Mage(text);
            if (mage.getContent().equals("10001")) {
                if (!InformationOperateMapMatching.login.containsKey(mage.getSender())) {
                    InformationOperateMapMatching.login.put(mage.getSender(), "");
                    InformationOperateMapMatching.offer(ctx, mage);
                    if (queue.size() >= 2) {
                        String chatRoom = UUID.randomUUID().toString();
                        InformationOperateMapMatching iom1 = queue.poll().setChatRoomId(chatRoom);
                        InformationOperateMapMatching iom2 = queue.poll().setChatRoomId(chatRoom);
                        InformationOperateMapMatching.add(iom1.getChannelHandlerContext(), iom1.getIMMessage());
                        InformationOperateMapMatching.add(iom2.getChannelHandlerContext(), iom2.getIMMessage());
                        iom1.sead(iom2.getIMMessage());
                        iom2.sead(iom1.getIMMessage());
                    }
                } else {//用户已登录
                    mage.setContent("-10001");
                    sendWebSocket(mage.toJson());
                    ctx.close();
                }
            } else {
                //将用户发送的消息发给所有在同一聊天室内的用户
                InformationOperateMapMatching.map.get(mage.getChatRoom()).forEach((id, iom) -> {
                    try {
                        iom.sead(mage);
                    } catch (Exception e) {
                        System.err.println(e);
            ...
            this.sessionId = mage.getSender();
            this.name = mage.getSender();
        } else {
