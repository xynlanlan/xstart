...
$(document).ready(function(){
	...
	window.CHAT = {
		...
		serverAddr: hosts[0]=='localhost'?"ws://" + hosts[0] + ":8899/im":"wss://" + host + "/im",
		sendMsg:{},
		//保存浏览器socket对象
		socket:null,
		...
		login:function(){
            ...
            if (nickname != "") {
				...
				CHAT.sendMsg.sendName=nickname;
        		$("#nickname").val('');
                $("#loginbox").hide();
                $("#chatbox").show();
                this.init();
		...
		sendText:function() {
		    ...
		    if (CHAT.socket.readyState == WebSocket.OPEN) {
				CHAT.sendMsg.type=3;
				CHAT.sendMsg.msg=message.html().replace(/\n/ig,"<br/>");
		    	CHAT.socket.send(JSON.stringify(CHAT.sendMsg));
		    	message.empty();
		    	message.focus();
		    } else {
		        alert("与服务器连接失败.");
		...
		sendFlower:function(){
		    ...
		    if (CHAT.socket.readyState == WebSocket.OPEN) {
				CHAT.sendMsg.type=3;
				CHAT.sendMsg.isFlower=true;
				CHAT.sendMsg.msg='送鲜花';
		    	CHAT.socket.send(JSON.stringify(CHAT.sendMsg));
		    	$("#send-message").focus();
		    } else {
		        alert("与服务器连接失败.");
		...
		init:function(){
			...
			$("#showId").val(CHAT.sendMsg.sendId);
			$("#shownikcname").html(CHAT.sendMsg.sendName);
			...
			//将消息添加到聊天面板
			var appendToPanel = function(cmd,msg){
				var data = JSON.parse(msg);
				cmd = data.isFlower?'FLOWER':cmd;
				CHAT.sendMsg.isFlower=false;
				if(data.type == 0){
					addSystemTip(data.msg);
				}else if(cmd == "SYSTEM"){
					addSystemTip(data.msg);
				}else if(cmd == "CHAT"){
					var date = new Date();
					addSystemTip('<span class="time-label">' + date.format("hh:mm:ss") + '</span>');
					var isme = (data.sendId==CHAT.sendMsg.sendId) ? true : false;
					var contentDiv = '<div>' + data.msg + '</div>';
					var usernameDiv = '<span>' + isme?data.sendName:data.friendName + '</span>';
			...
			if (window.WebSocket) {
			    ...
			    CHAT.socket.onmessage = function(e) {
					appendToPanel("CHAT",e.data);
			    };
			    CHAT.socket.onopen = function(e) {
					CHAT.sendMsg.type=1;
					CHAT.sendMsg.sendId=CHAT.guid();
					CHAT.sendMsg.msg='我上线了';
					CHAT.socket.send(JSON.stringify(CHAT.sendMsg));
			    };
			    CHAT.socket.onclose = function(e) {
					CHAT.sendMsg.type=1;
					CHAT.sendMsg.msg='服务器关闭,暂不能聊天';
					appendToPanel("SYSTEM",JSON.stringify(CHAT.sendMsg));
			    };
			} else {
